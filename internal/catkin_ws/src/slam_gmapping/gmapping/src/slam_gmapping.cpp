...
SlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  ...
  //ros::Time timeclock = ros::Time::now();
  //std::cout << "[ INFO] [" << ros::Time::now() << "]: main catch" << std::endl;
  //ROS_INFO("%s", timeclock);
  //m_infoStream << timeclock << endl;
  /////////////////////////////////////////////////
  ...
  if(addScan(*scan, odom_pose))
  {
    ...
    //akira// GET particles information (index, pose, weight) //akira//
    /*
    int particleindex = 0;
    for(std::vector<GMapping::GridSlamProcessor::Particle>::const_iterator it = gsp_->getParticles().begin();
        ...
        ++it)
    {
      GMapping::OrientedPoint mpose2 = gsp_->getParticles()[particleindex].pose;
      ROS_INFO("Pose[%d] : %.3f %.3f %.3f", particleindex, mpose2.x, mpose2.y, mpose2.theta);
      //weight_total2 += it->weight;
      ROS_INFO("PoseWeight: %.5f", it->weight);
      //
      GMapping::OrientedPoint pmpose2 = gsp_->getParticles()[particleindex].previousPose;
      ROS_INFO("Previous[%d] : %.3f %.3f %.3f", it->previousIndex, pmpose2.x, pmpose2.y, pmpose2.theta);
      //ROS_INFO("PreviousIndex: %d", it->previousIndex);
      //ROS_INFO("PoseWeight: %.5f", it->weight);
      particleindex++;
    }
    */
...
SlamGMapping::computePoseEntropy()
{
      ...
      ++it)
  {
    ...
    //ROS_INFO("PoseWeight: %.10f", it->weight);  ////////////////////////
  }
  double entropy = 0.0;
  for(std::vector<GMapping::GridSlamProcessor::Particle>::const_iterator it = gsp_->getParticles().begin();
      it != gsp_->getParticles().end();
